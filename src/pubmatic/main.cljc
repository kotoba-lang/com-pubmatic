(ns pubmatic.main
  "Kotodama WASM entrypoint for the Pubmatic clean-room actor (L4) — Clojure port.
  ...
  facts (`pubmatic.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pubmatic")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"pubmatic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pubmatic-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pubmatic-compat/ready)
